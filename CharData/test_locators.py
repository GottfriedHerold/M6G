...
def test_lookup_order(empty3cv: CharVersion):
    ...
    assert not list(*it2)
    it3 = empty3cv.lookup_candidates('_all', lists=[1])
    l3 = list(it3)
    assert l3 == [('_all', 1)]
    l4 = list(empty3cv.lookup_candidates('a.__b__.c', lists=[1]))
    assert l4 == [('a.__b__.c', 1), ('a.__b__._all', 1)]
    ll = list(empty3cv.lookup_candidates('__a__.b.c', lists=[1]))
    assert ll == [('__a__.b.c', 1), ('__a__.b._all', 1), ('__a__.c', 1), ('__a__._all', 1)]
    ll = list(empty3cv.lookup_candidates('a.b.__c__', lists=[1]))
    ...
    ll = list(empty3cv.function_candidates('g', lists=[1]))
